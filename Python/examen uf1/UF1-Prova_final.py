...
Nom i Cognoms: Adria Martinez Hernandez
DNI: 48077277D
URL vídeo: https://drive.google.com/file/d/1GCIru59EqsCfudQvZ4aBzgePc7b3r-uo/view?usp=sharing 
...
students = {}
def createCode():
    lletres = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P", "Q", "S", "R", "T", "W", "V", "X", "Y", "Z"]
    code = ""
    for cnt in range(3):
    ...
    code = code + "-"
    for cnt in range(2):
        num_naimxament = random.randint(0, 5)
        code = code + str(num_naimxament)
    return code

def createName():
    noms = ["Adria", "Alejandro", "Marina", "Albert", "Paula", "Jose", "Carlos", "Carlota", "Pepe", "David"]
...
def createGames():
    jocs = ["Tennis", "Ping-Pong", "Indoor Hockey", "Football", "Chess", "Swimming", "Basketball", "Volleyball", "Handball"]
    jocs_estudiant = []
    num_jocs = random.randint(2 ,5)
    for cnt in range(0, num_jocs):
        joc = random.choice(jocs)
        jocs_estudiant.append(joc)
        jocs.remove(joc)
    return jocs_estudiant
    ...
    for cnt in range(0, number):
        ...
        student = {
            codi: {
                ...
                "name": createName(),
                "age": createAge(),
                "antiqueitey": createAntiquitey(),
                "games": createGames()
            }
        }
        codis.append(codi)
        students.update(student)
    afegeixPrompt(f"(createDatabase) {number} students added")
    return codis
...
def searchStudents(param, value):
    ...
    for key in students.keys():
        codi = key
        if value == students[key][param]:
            codis.append(codi)
    afegeixPrompt(f"(searchStudents) Found {len(codis)}: {codis}")
    for cnt in range(0, len(codis)):
        student = students[codis[cnt]]
        afegeixPrompt(student)
    return codis
...
def modifyStudents(code, param, value):
    if code in students:
        ...
        afegeixPrompt(f"(modifyStudents) Student {code} modified")
    else:
        afegeixPrompt(f"(modifyStudents) Student {code} not found")
    return
...
def deleteStudents(list):
    ...
    afegeixPrompt(f"(deleteStudents) Students deleted: {esborrats}")
    return esborrats
...
def listStudents(param):
    ...
    for cnt in range(0, len(llista_ordenada)):
        afegeixPrompt(llista_ordenada[cnt])
...
def AlimentaB1(B1, CA):
    for cnt in range(0, B1):
        if CA - 3 > 0:
            CA = CA - 3
        else:
            B1 = B1 - 1
...
def evolucioCelulas():
    ...
    B1 = generaB1()
    CA = generaCA()
    print("Seconds       Bacteria B1       Cells CA")
    while B1 > 0 and CA > 0:
        print()
